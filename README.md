...
На VPS активен и включён `wdtt-server.service`; бинарь расположен в
`/opt/wdtt/wdtt-server`, конфигурация — в `/etc/wdtt/`, внутренний WireGuard-порт —
`56011/udp`. Подробная локальная карточка:
`../Servera_vpn_andmo/servers/vps-185-245-34-224.md`.

Живой сервер пока работает на совместимой версии `v1.2.0`, поэтому в клиенте
нужно оставлять маскировку `audio`. Новый режим `video` (`PT=96`) требует
отдельного обновления серверного бинаря. Пароль сервера в репозитории не хранится.
